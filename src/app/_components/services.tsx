    ...
    <section className="bg-white py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          <div className='overflow-hidden' ref={emblaRef}>
            <div className='flex'>
              {services.map((item, index) => (
                <div
                  key={index}
                  className='flex-[0_0_100%] min-w-0 md:flex-[0_0_33%] px-3'>
                  <article className='bg-[#1e293b] text-white rounded-2xl p-6 space-y-4 h-full flex flex-col'>
                    ...
                    <div className='border-t border-gray-700 pt-4 flex items-center justify-between'>
                      ...
                      <a
                        href={`https://wa.me/5548999990000?text=Olá, vim pelo site e gostaria de mais informações sobre ${item.title}`}
                        target="_blank"
          ...
          <button className='bg-white flex items-center justify-center rounded-full shadow-lg w-10 h-10 absolute left-4 -translate-y-1/2 -translate-x-1/2 top-1/2 z-10'>
            <ChevronLeft
              onClick={scrollPrev}
              className='w-6 h-6 text-gray-600 ' />
          </button>

          <button className='text-gray-600 bg-white flex items-center justify-center rounded-full shadow-lg w-10 h-10 absolute -right-4 -translate-y-1/2 -translate-x-1/2 top-1/2 z-10'>
            <ChevronRight
              onClick={scrollNext}
              className='w-6 h-6 text-gray-600' />
          </button>
